...
public class Track {
    ...
    public void doTrackWork(long startTimestamp) {
        //到了下个乐符开始的时间没
        if(beats.peek()!= null && (beats.peek().rightTime - animateTime <= new Date().getTime()-startTimestamp)) {
            //如果是，取乐符，判断类型，播放滑行动画。
            System.out.println("Beat 出现在屏幕上");
            curBeat.add(beats.poll());
        }
        //到了当前最先的乐符结束时间没
        if((curBeat.peek()!= null) && (curBeat.peek().rightTime + delayTime > new Date().getTime()-startTimestamp)) {
